...
4. 让规则 planner 和 LLM planner 生成受 schema 约束的结构化补丁。
5. 通过 FastAPI 和 React 工作台完成多轮理解、设计摘要、受控改造、变更可视化、需求覆盖门禁和导出验收。

## 当前能力

已推进到 Deadline P0 第 7 天缓冲和演示固化阶段：
...
- 工作流已接入 `RequirementSlots` 归一化层，能合并规则摘要和 LLM 需求抽取结果，支持多轮覆盖/否定语义、来源/状态记录和已有工程后的局部修改路由。
- 工作流已接入确定性结构化需求摘要，能在模板选择前识别项目类型、设备、控制功能、通讯/IO、保护逻辑和待澄清问题。
- 模板检索后会生成轻量 `DesignBrief`，展示推荐模板、已满足项、需改造项、设备/控制/点表/保护计划、风险项和导出门禁提示。
- 校验和导出已接入 `RequirementConformanceReport`，有需求上下文时会执行结构安全和需求覆盖双门禁；找不到上下文时明确提示“需求覆盖未复核”。
...
- 支持 LLM 结构化补丁规划，并立即进入 dry-run；LLM 只输出受 schema 约束的补丁意图，实际修改仍由 patch engine 执行。
- 规则 planner 仍优先处理明确低风险小修改；规则无法规划且命中结构改造意图时，后端会自动 fallback 到 LLM planner dry-run feedback，并在不可用时返回可执行追问。
...
- 提供 FastAPI API。
- 提供 FastAPI 托管的 React 工作台，支持需求槽位、模板候选、设计摘要、风险确认、覆盖报告、版本历史、版本 diff、回滚、局部 React Flow 图、affected 节点定位、diff/risk 高亮、Trace、事件流、指标、成本、趋势和反馈。
...
- 已有 pytest、Vitest 和 Playwright 验收覆盖服务层、工作流、API、前端组件和真实浏览器主流程。
- 评测集已覆盖模板选择、需求抽取、多轮覆盖/否定、planner 结构化补丁和 6 条业务验收样例。
...
- 复合功能块配方和更多真实项目样例校准。
- 向量库/混合检索。
- PostgreSQL 权限模型、租户隔离、运维查询和更完整生产化配套。
- 完整生产部署配套，例如鉴权、权限、CORS/HTTPS、限流、备份恢复和审计检索。
...
- 首屏选择 `机房群控程序` 或 `AHU 程序`，点击 `开始会话` 或左侧 `新建` 创建会话。
- 输入项目需求。
- 查看结构化需求清单、需求来源状态、多轮覆盖/否定结果和待澄清问题。
- 展示并确认模板候选。
- 查看设计摘要、模板能力解释、需改造项和风险点。
- 创建工程版本。
- 规划补丁。
- dry-run 中高风险结构化补丁并人工确认。
- 补丁成功后切换到新的工程版本。
- 查看校验结果、需求覆盖报告和导出阻塞原因。
- 查看局部流程图、diff 分类、affected 节点定位和风险高亮。
- 查看版本历史、任意版本对比、回滚、Trace、事件流、指标、成本、趋势和反馈。
- 导出最终 JSON。
...
独立 planner API 可以显式传入 `use_llm: true`。工作流会话默认启用 LLM planner；规则 planner 失败且识别为复杂结构改造意图时，即使前端开关关闭，后端也会尝试自动 fallback 到 LLM dry-run feedback。示例：
...
工作流消息接口可传入 `use_llm_planner` 覆盖会话默认值；创建会话时也可设置：
...
- `tests/test_requirement_extraction_evals.py`：需求槽位抽取、多轮覆盖/否定和高风险语义评测。
- `tests/test_planner_evals.py`：LLM planner 评测样例和真实 DeepSeek 结构化规划验收。
- `tests/test_business_acceptance.py`：Deadline 第 6 天业务验收，覆盖设计、改造、覆盖门禁、高风险确认和局部图定位。
...
1. 按 [文档/演示脚本.md](文档/演示脚本.md) 固化演示路径。
2. 继续按真实项目样例细化通讯读写方向、更多设备保护链路和业务例外白名单。
3. 只做 bug fix、真实样例校准和演示文案打磨，不再扩大为从零生成完整 JSON。
